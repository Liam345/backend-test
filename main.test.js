...
const {groupBy, sumBy, chunk, luhnCheck}  = require("./main");
...
describe("Chunk", () => {
  test("3a Chunk", () => {
    const arraySize = 1e6;
    const chunkSize = 5;
    const chunkArraySize = Math.ceil(arraySize / chunkSize);

    const array = [...Array(arraySize).keys()];

    const result = chunk(array, chunkSize);

    expect(result).toHaveLength(chunkArraySize);
  });
});
